...
- **Pause-aware captions** (`split_segments_at_pauses`/`_phrase_split` in segmenter.py): captions are cut at reciter breath pauses — inter-word silences ≥ `SILENCE_SPLIT` (0.4s) from forced-aligned word times (local path) or provider word timestamps (provider path, incl. opener segments). Each phrase becomes its own segment; canon AR text is re-sliced proportionally by word count (recitation order == text order), EN by proportional cuts snapped to sentence-break punctuation (`_snap_en_boundary`). Audio-evidenced cuts are ALWAYS kept (even 1-word phrases — show briefly, then blank, never bridge a pause); tiny-phrase merging applies to the whisper-gap fallback path only. `snap_words_to_silence` also truncates drifted word stamps that contain/swallow a breath (containment + midpoint-shrink rules) so no word can hold the overlay up mid-pause. `apply_sync_padding(segments, silence_runs)` enforces non-overlap with lead-in 30ms / lead-out 120ms, clamped to silence edges so padding never bleeds into a pause. Empty `silence_runs` (VAD: truly continuous) never falls back to whisper gaps. Player hides captions frame-accurately via rAF loop + exclusive end (`t < end`). Verified 2026-09-08 on full 2h Baqarah (real groq words + Silero VAD): 83/83 pauses ≥0.4s blank, 0.00s caption-in-pause (was 48 pauses / 39.0s); live /align on 60s Ikhlas+Kawthar clip: 9/9 pauses blank; player overlay probed in real browser (text at t=12, empty at t=15.5/20.3/42.0, exact-boundary correct, pause-freeze correct).
